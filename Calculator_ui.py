...
import taichi as ti
ti.init(arch=ti.cpu) # Initialize Taichi, you can choose different backends like cuda or vulkan
...
# Taichi function for demonstration
@ti.kernel
def taichi_square(x: ti.f32) -> ti.f32:
    return x * x
...
def taichiButton():
    try:
        ...
        taichi_result = taichi_square(value)
        inputText.set(str(taichi_result))
    except ValueError:
        inputText.set("Invalid Input for Taichi")
    except Exception as e:
        inputText.set(f"Taichi Error: {e}")
...
pi_btn = PhotoImage(file = r"images\pi.png")
piimage = pi_btn.subsample(4,4)
pi_btn = Button(mainFrame, text="pi", fg="black",image=piimage, bd=0, bg="black", cursor="hand2",
...
# Taichi Button
taichi_btn = Button(mainFrame, text="Taichi\nSquare", fg="black", bd=0, bg="lightgreen", cursor="hand2",
               command=lambda: taichiButton())
taichi_btn.grid(row=7, column=0, columnspan=2, padx=1, pady=1, sticky="ew") # span two columns and sticky to fill space
